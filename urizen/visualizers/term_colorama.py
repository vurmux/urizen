...
class VisTermColorama(object):
    ...
    def visualize(self, M):
        for line in M.cells:
            for cell in line:
                print(
                    (
                        self._transform_color(cell.bg_color, is_fore=False) +
                        self._transform_color(cell.fg_color) +
                        cell.symbol
                    ),
                    end=''
                )
    ...
    def _transform_color(self, rgb_color, is_fore=True):
        if rgb_color == '#000000':
